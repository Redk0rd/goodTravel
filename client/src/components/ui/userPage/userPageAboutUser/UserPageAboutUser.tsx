...
import UserEditModal from '../userPageEditModal/UserEditModal';
          ...
          <Box className="width_userPage">
            <Box className="ava_container">
              ...
              <Avatar
                size="4xl"
                className="avatar"
                src={`${import.meta.env.VITE_APP_BASE_IMG}/${user?.pathImg}`}
              />
              {/* <button type="button" className="btn">Редактировать</button> */}
              <button type="button" className="Btn" onClick={() => void dispatch(setUser(user))}>
                Редактировать
            ...
            <Box className="text_container">
              <Flex className="flex_column text_shadow">
                ...
                <Text mt={3} className="text_description">
                  О себе: {user.about}
                </Text>
              </Flex>
            </Box>
